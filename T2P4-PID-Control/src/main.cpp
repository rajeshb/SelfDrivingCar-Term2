...
int main()
{
    ...
    PID sPid;
    double throttle_Kp = 0.2;
    double throttle_Ki = 0.0;
    double throttle_Kd = 0.0;
    sPid.Init(throttle_Kp, throttle_Ki, throttle_Kd);
    
    // Desired speed
    double desired_speed = 30.0;
    
    h.onMessage([&pid, &sPid, desired_speed](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2')
        {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    sPid.UpdateError(speed_error);
                    double throttle_value = sPid.TotalError();
